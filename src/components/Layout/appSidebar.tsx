      ...
      <Sidebar className="h-screen flex flex-col border-r bg-white">
        ...
        <SidebarContent>
          <SidebarGroup>
            ...
            <SidebarMenu>
              {navItems.map((item) => {
              const isActive = pathname === item.url; // ✅ define isActive
                ...
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild>
                    <button
                      ...
                      className={`flex items-center w-full py-3 space-x-2 rounded transition-colors
                        ${isActive 
        ...
        <SidebarFooter className="flex flex-col items-center py-4"> 
          ...
          <p className="text-xs text-white/80">{user?.constituency}</p>
           ...
           <Button onClick={() => signOut({ redirect: false }).then(() => { window.location.href = '/'; })} variant="ghost" size="sm" className="w-full text-red-600 hover:text-red-700 hover:bg-red-50"> <LogOut className="w-4 h-4 mr-2"/> Log Out </Button> </SidebarFooter>
      </Sidebar>
  );
}
